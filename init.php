...
if ( learn_press_buddypress_is_active() ) {
	...
	function learn_press_buddy_press_profile( $methods ) {
		$methods['bp_profile'] = __( 'BuddyPress Profile', 'learnpress_buddypress' );
		return $methods;
	}
	...
	if ( learn_press_has_buddypress_profile_method() ) {
		...
		function learn_press_bp_courses_name() {
			return __( 'Courses', 'learnpress_buddypress' );
		...
		function learn_press_bp_courses_nav_all( $sub_navs ) {
			$nav_all = array(
				'name'                    => __( 'Course', 'learnpress_buddypress' ),
				'slug'                    => 'all',
				'show_for_displayed_user' => true,
				'position'                => 10,
				'screen_function'         => 'learn_press_bp_courses_all',
		...
		function learn_press_bp_courses_nav_quiz_results( $sub_navs ) {
			$nav_all = array(
				'name'                    => __( 'Quiz results', 'learnpress_buddypress' ),
				'slug'                    => 'quiz_results',
		...
		function learn_press_bp_courses_bar( $wp_admin_nav ) {
			...
			$wp_admin_nav[] = array(
				...
				'title'  => __( 'All courses', 'learnpress_buddypress' ),
			...
			$wp_admin_nav[] = array(
				...
				'title'  => __( 'Quiz Results', 'learnpress_buddypress' ),
		...
		function learn_press_bp_courses_all_title() {
			echo __( 'All courses', 'learnpress_buddypress' );
		}
		...
		function learn_press_bp_courses_quiz_results_title() {
			echo __( 'Quiz Results', 'learnpress_buddypress' );
		}
		...
		add_filter( 'learn_press_instructor_profile_link', 'learn_press_get_bp_link', 20, 3 );
		function learn_press_get_bp_link( $link, $user_id, $course_id ) {
			// Determine user to use
			if ( is_null( $user_id ) ) {
